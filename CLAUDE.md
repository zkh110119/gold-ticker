...
| In progress | R9 — GitHub Release automation | Added a tag-triggered GitHub Actions workflow that validates the exact Cargo version, runs the release quality suite, builds and verifies the unsigned and unnotarized `.app`, `.dmg`, and SHA-256 sidecar on `macos-14`, and creates a GitHub Release with the matching DMG and checksum assets. Added a protected `release` environment hook and documented the automated tag workflow while keeping Tencent data-source authorization and other publication prerequisites as explicit release gates. No signing, notarization, Mac App Store submission, or Gatekeeper bypass instructions were added. | Workflow YAML parses and local validation passes: `git diff --check`, `cargo fmt --check`, `cargo test` (35 passed), `cargo clippy --all-targets -- -D warnings`, and `cargo build --release`. The workflow is being repaired after the tag-triggered run exposed that Rust 1.85.0 cannot compile the current dependency lockfile; the toolchain and declared MSRV are being aligned to Rust 1.88. |

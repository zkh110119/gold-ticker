...
| Completed | R9 — GitHub Release automation | Added a tag-triggered GitHub Actions workflow that validates the exact Cargo version, runs the release quality suite, builds and verifies the unsigned and unnotarized `.app`, `.dmg`, and SHA-256 sidecar on `macos-14`, and creates a GitHub Release with the matching DMG and checksum assets. Added a protected `release` environment hook and documented the automated tag workflow while keeping Tencent data-source authorization and other publication prerequisites as explicit release gates. No signing, notarization, Mac App Store submission, or Gatekeeper bypass instructions were added. | Workflow YAML parses and validation passes: `git diff --check`, `cargo fmt --check`, `cargo test` (35 passed), `cargo clippy --all-targets -- -D warnings`, and `cargo build --release`. The tag-triggered Rust toolchain issue was resolved by aligning the workflow and declared MSRV with the locked dependencies at Rust 1.88. |
| Completed | R10 — Quote presentation and menu interaction | Preserved three decimal places for percentage changes; changed the default threshold to `4000` and labeled it as `美元`; opened the Popover on a primary-button click with outside-click dismissal; added a secondary-click context menu with an explicit quit action. Also requests notification authorization when a threshold is saved, establishes the current quote as the baseline when available, exposes packaged-app notification failures for diagnosis, and synchronizes the context menu with the current system appearance each time it opens. | `cargo fmt --check`, `cargo test` (36 passed), `cargo clippy --all-targets -- -D warnings`, and `cargo build --release` pass. The ad-hoc signed release app bundle and DMG build successfully; `Info.plist` is valid with display name `Gold Ticker`, bundle name `Gold Ticker`, and identifier `com.goldticker.app`. Native verification confirmed percentage formatting, `4,000 美元` threshold presentation, primary-click Popover behavior, outside-click dismissal, secondary-click quit, notification authorization and listing, threshold notification behavior, duplicate suppression and re-alert after recovery, and light/dark context-menu synchronization after runtime appearance changes. |
| Completed | R11 — DMG Finder installation window layout | Replaced unreliable Finder/AppleScript mutation with the documented `dmgbuild` declarative layout: local and `macos-14` builds install pinned `dmgbuild==1.6.5`, generate the final DMG directly with stable volume name, window bounds, icon size, and icon locations, and retain the drag-to-Applications model. The background arrow points cleanly to Applications without extending beyond its head, and both icons are centered vertically. | `git diff --check`, `cargo fmt --check`, `cargo test` (36 passed), `cargo clippy --all-targets -- -D warnings`, and `cargo build --release` pass; the local build produced the final DMG; `hdiutil verify`, read-only remount/content, `.DS_Store`, and SHA-256 checks pass; user-confirmed Finder visual verification approved the centered icons and corrected arrow presentation. |
